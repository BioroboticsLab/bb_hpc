...
##### DETECT
#################################################################
def generate_jobs_detect(video_root_dir,
                         repo_output_path,
    ...
    # 1) figure out which videos are already queued
    ...
    # 2) glob all your raw mp4’s
    all_videos = []
    for day in datestring:                    # e.g. "20250618", "20250619"
    ...
    # 4) filter out videos already processed _or_ already pending
    to_schedule = []
    for v in all_videos:
        # skip if already queued
        if v in pending:
            continue
        # where the .bbb _should_ be
        rel = get_bbb_file_path(os.path.basename(v))
        full_bbb = os.path.join(repo_output_path, rel)
        # skip if already processed
        if os.path.exists(full_bbb):
            if verbose:
                print('already exists:', rel)
            continue
        if verbose:
            print('adding:',v)
    ...
    ## using normal order (note: reverse sort would make newest process first)
    to_schedule = sorted(to_schedule, reverse=False)

    ## keep only videos for max number of jobs
    if maxjobs is not None:
        to_schedule = to_schedule[0:chunk_size*maxjobs]

    print('number of videos kept with max jobs',len(to_schedule))
    
    # 5) parse timestamps, sort, and chunk
    video_tuples = []
    for v in to_schedule:
        cam_id, dt_start, _ = parse_video_fname(os.path.basename(v),
                                                format=video_name_format)
        video_tuples.append((dt_start, cam_id, v))
    video_tuples.sort()

    # 6) now yield in batches of size chunk_size
    for i in range(0, len(video_tuples), chunk_size):
        chunk = video_tuples[i:i+chunk_size]
        yield {
            "video_paths": [path for _, _, path in chunk],
            "repo_output_path": repo_output_path,   # <— add this
        }        
